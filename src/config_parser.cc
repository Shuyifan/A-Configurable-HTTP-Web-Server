...
bool NginxConfigParser::Parse(std::istream* config_file, NginxConfig* config) {
  ...
  while (true) {
    ...
    if (token_type == TOKEN_TYPE_START) {
      ...
      if (config_stack.top()->statements_.back().get()->tokens_.size() < 2) {
        // Error.
        break;
      }
      ...
      if (last_token_type != TOKEN_TYPE_STATEMENT_END &&
          last_token_type != TOKEN_TYPE_END_BLOCK) {
        // it is possible that one end_block is after another end_block
        // like the config file in the test
        // Error.
        break;
      }
      if(config_stack.size() < 2) {
        break;
      }
      config_stack.pop();
